...
def test_complaint_schema_validates_content_and_status() -> None:
    ...
    assert not hasattr(item, "scene")

    with pytest.raises(ValidationError):
        ComplaintCreateIn(target_user_id=0, reason="其他", content="x")

    with pytest.raises(ValidationError):
        ComplaintCreateIn(target_user_id=2, reason="其他", content="   ")
...
def test_complaint_model_routes_and_admin_contract_exist() -> None:
    ...
    assert "target_pending_complaint_count" not in schema_text
    assert "target_risk_flag" not in schema_text
    ...
    assert "scene: str = Query" not in admin_api_text
    assert "Q(scene=" not in admin_api_text
    assert "target_pending_complaint_count" not in admin_api_text
    assert "target_risk_flag" not in admin_api_text
    assert "multiple_complaints" not in admin_api_text
...
def test_complaint_migration_and_admin_web_exist() -> None:
    ...
    assert "DROP COLUMN `scene`" in migration_text or "DROP COLUMN scene" in migration_text
    ...
    assert "来源" not in web_view_text
    assert "待处理投诉次数" not in web_view_text
    assert "风险标识" not in web_view_text
    assert "多次被投诉" not in web_view_text
    assert "处理投诉不会自动封禁用户" in web_view_text
    assert "查看用户" in web_view_text
    assert "function viewUser" in web_view_text
    assert "viewUser(row.complainant_id)" in web_view_text
    assert "viewUser(row.target_user_id)" in web_view_text
    assert "/operation/app-user" in web_view_text
    assert "user_id" in web_view_text
