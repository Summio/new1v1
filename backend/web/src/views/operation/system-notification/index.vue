...
async function fetchData(params = {}) {
  const res = await api.getSystemNotificationList({
    page: params.page,
...
async function estimateTargetCount() {
  try {
    ...
    const res = await api.estimateSystemNotificationTargetCount({
    ...
    window.$message?.error(error?.message || '预计触达人数失败')
  }
}
...
async function handleDelete(row) {
  try {
    await api.deleteSystemNotification({ id: row.id })
...
const columns = [
  ...
  },
  {
    ...
    width: 120,
    render(row) {
      return labelOf(sendModeOptions, row.send_mode)
    },
  },
  {
    title: '状态',
        ...
        actions.push(
          h(
            NButton,
            {
              size: 'small',
              secondary: true,
              onClick: () => doAction(row, 'pauseSystemNotification', '已暂停'),
            },
            { default: () => '暂停' }
          )
        )
        ...
        actions.push(
          h(
            NButton,
            {
              size: 'small',
              secondary: true,
              onClick: () => doAction(row, 'cancelSystemNotification', '已取消'),
            },
            { default: () => '取消' }
          )
        )
...
<template>
  <CommonPage show-footer title="系统通知">
    <CrudTable
      ref="$table"
