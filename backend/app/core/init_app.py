...
async def init_roles():
    ...
    fee_bill_list_api = await Api.filter(method="GET", path="/api/v1/fee_bill/list").first()
    if fee_bill_list_api and all_roles:
        for role in all_roles:
            await role.apis.add(fee_bill_list_api)
    ...
    withdraw_apis = await Api.filter(
        path__in=[
            ...
            "/api/v1/withdraw/account/list",
            "/api/v1/withdraw/account/review",
        ],
    ).all()
