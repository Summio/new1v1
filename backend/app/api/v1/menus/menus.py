...
async def list_menu(
    ...
    # 一次性加载所有菜单，构建树（避免递归 N+1 查询）
    all_menus = await menu_controller.model.all().order_by("order")
    menus_by_id = {m.id: await m.to_dict() for m in all_menus}
    for m in all_menus:
        menus_by_id[m.id]["children"] = []
    root_menus = []
    for m in all_menus:
        if m.parent_id == 0:
            root_menus.append(menus_by_id[m.id])
        else:
            parent = menus_by_id.get(m.parent_id)
            if parent is not None:
                parent["children"].append(menus_by_id[m.id])
    res_menu = root_menus
    total = len(res_menu)
    # 简单分页：直接切片
    start = (page - 1) * page_size
    end = start + page_size
    return SuccessExtra(data=res_menu[start:end], total=total, page=page, page_size=page_size)
